...
from graphx.detect import classify_file, count_words, detect, FileType, _looks_like_paper, _is_ignored, _load_graphxignore
...
def test_classify_attention_paper():
    ...
    paper_path = Path("/home/safi/graphx_eval/papers/attention_is_all_you_need.md")
    if paper_path.exists():
        result = classify_file(paper_path)
        assert result == FileType.PAPER


def test_graphxignore_excludes_file(tmp_path):
    """Files matching .graphxignore patterns are excluded from detect()."""
    (tmp_path / ".graphxignore").write_text("vendor/\n*.generated.py\n")
    ...
    assert result["graphxignore_patterns"] == 2


def test_graphxignore_missing_is_fine(tmp_path):
    """No .graphxignore is not an error."""
    (tmp_path / "main.py").write_text("x = 1")
    result = detect(tmp_path)
    assert result["graphxignore_patterns"] == 0


def test_graphxignore_comments_ignored(tmp_path):
    """Comment lines in .graphxignore are not treated as patterns."""
    (tmp_path / ".graphxignore").write_text("# this is a comment\n\nmain.py\n")
    (tmp_path / "main.py").write_text("x = 1")
...
def test_graphxignore_hermetic_without_vcs(tmp_path):
    """Without a VCS root, parent .graphxignore does NOT apply (hermetic)."""
    (tmp_path / ".graphxignore").write_text("vendor/\n")
    ...
    # parent .graphxignore must NOT leak into a non-VCS scan
    assert any("vendor" in f for f in code_files)
    assert result["graphxignore_patterns"] == 0


def test_graphxignore_discovered_from_parent_in_vcs(tmp_path):
    """Inside a VCS repo, parent .graphxignore applies to subdirectory scans."""
    (tmp_path / ".git").mkdir()
    (tmp_path / ".graphxignore").write_text("vendor/\n")
    sub = tmp_path / "packages" / "mylib"
    sub.mkdir(parents=True)
    ...
    assert result["graphxignore_patterns"] >= 1


def test_graphxignore_stops_at_git_boundary(tmp_path):
    """Upward search stops at the git repo root (.git directory)."""
    (tmp_path / ".graphxignore").write_text("main.py\n")
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / ".git").mkdir()
    sub = repo / "sub"
...
def test_graphxignore_at_git_root_is_included(tmp_path):
    """A .graphxignore at the git repo root is included when scanning a subdir."""
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / ".git").mkdir()
    (repo / ".graphxignore").write_text("vendor/\n")
...
def test_classify_video_extensions():
    ...
    from graphx.detect import FileType
