"""Tests for graphx install --platform routing."""
...
PLATFORMS = {
    "claude": (".claude/skills/graphx/SKILL.md",),
    "codex": (".agents/skills/graphx/SKILL.md",),
    "opencode": (".config/opencode/skills/graphx/SKILL.md",),
    "claw": (".openclaw/skills/graphx/SKILL.md",),
    "droid": (".factory/skills/graphx/SKILL.md",),
    "trae": (".trae/skills/graphx/SKILL.md",),
    "trae-cn": (".trae-cn/skills/graphx/SKILL.md",),
    "windows": (".claude/skills/graphx/SKILL.md",),
}


def _install(tmp_path, platform):
    from graphx.__main__ import install
    with patch("graphx.__main__.Path.home", return_value=tmp_path):
        install(platform=platform)


def test_install_default_claude(tmp_path):
    _install(tmp_path, "claude")
    assert (tmp_path / ".claude" / "skills" / "graphx" / "SKILL.md").exists()


def test_install_codex(tmp_path):
    _install(tmp_path, "codex")
    assert (tmp_path / ".agents" / "skills" / "graphx" / "SKILL.md").exists()


def test_install_opencode(tmp_path):
    _install(tmp_path, "opencode")
    assert (tmp_path / ".config" / "opencode" / "skills" / "graphx" / "SKILL.md").exists()


def test_install_claw(tmp_path):
    _install(tmp_path, "claw")
    assert (tmp_path / ".openclaw" / "skills" / "graphx" / "SKILL.md").exists()


def test_install_droid(tmp_path):
    _install(tmp_path, "droid")
    assert (tmp_path / ".factory" / "skills" / "graphx" / "SKILL.md").exists()


def test_install_trae(tmp_path):
    _install(tmp_path, "trae")
    assert (tmp_path / ".trae" / "skills" / "graphx" / "SKILL.md").exists()


def test_install_trae_cn(tmp_path):
    _install(tmp_path, "trae-cn")
    assert (tmp_path / ".trae-cn" / "skills" / "graphx" / "SKILL.md").exists()
...
def test_codex_skill_contains_spawn_agent():
    ...
    import graphx
    skill = (Path(graphx.__file__).parent / "skill-codex.md").read_text()
...
def test_opencode_skill_contains_mention():
    ...
    import graphx
    skill = (Path(graphx.__file__).parent / "skill-opencode.md").read_text()
...
def test_claw_skill_is_sequential():
    ...
    import graphx
    skill = (Path(graphx.__file__).parent / "skill-claw.md").read_text()
...
def test_all_skill_files_exist_in_package():
    ...
    import graphx
    pkg = Path(graphx.__file__).parent
...
def _agents_install(tmp_path, platform):
    from graphx.__main__ import _agents_install as _install_fn
    _install_fn(tmp_path, platform)


def _agents_uninstall(tmp_path, platform=""):
    from graphx.__main__ import _agents_uninstall as _uninstall_fn
...
def test_codex_agents_install_writes_agents_md(tmp_path):
    ...
    assert "graphx" in agents_md.read_text()
...
def test_agents_uninstall_removes_section(tmp_path):
    ...
    agents_md = tmp_path / "AGENTS.md"
    # File deleted when it only contained graphx section
...
def test_opencode_agents_install_writes_plugin(tmp_path):
    """opencode install writes .opencode/plugins/graphx.js."""
    _agents_install(tmp_path, "opencode")
    plugin = tmp_path / ".opencode" / "plugins" / "graphx.js"
...
def test_opencode_agents_install_registers_plugin_in_config(tmp_path):
    ...
    assert any("graphx.js" in p for p in config.get("plugin", []))
...
def test_opencode_agents_install_merges_existing_config(tmp_path):
    ...
    assert any("graphx.js" in p for p in config["plugin"])
...
def test_opencode_agents_uninstall_removes_plugin(tmp_path):
    ...
    if config_file.exists():
        config = _json.loads(config_file.read_text())
        assert not any("graphx.js" in p for p in config.get("plugin", []))


# ── Cursor ────────────────────────────────────────────────────────────────────

def test_cursor_install_writes_rule(tmp_path):
    """cursor install writes .cursor/rules/graphx.mdc."""
    from graphx.__main__ import _cursor_install
    _cursor_install(tmp_path)
    rule = tmp_path / ".cursor" / "rules" / "graphx.mdc"
    assert rule.exists()
    content = rule.read_text()
    assert "alwaysApply: true" in content
    assert "graphx-out/GRAPH_REPORT.md" in content
...
def test_cursor_uninstall_removes_rule(tmp_path):
    ...
    from graphx.__main__ import _cursor_install, _cursor_uninstall
    _cursor_install(tmp_path)
...
def test_cursor_uninstall_noop_if_not_installed(tmp_path):
    ...
    from graphx.__main__ import _cursor_uninstall
...
def test_gemini_install_writes_gemini_md(tmp_path):
    from graphx.__main__ import gemini_install
    gemini_install(tmp_path)
    md = tmp_path / "GEMINI.md"
    assert md.exists()
    assert "graphx-out/GRAPH_REPORT.md" in md.read_text()
...
def test_gemini_install_writes_hook(tmp_path):
    ...
    assert any("graphx" in str(h) for h in hooks)
...
def test_gemini_install_idempotent(tmp_path):
    ...
    gemini_install(tmp_path)
    md = tmp_path / "GEMINI.md"
    assert md.read_text().count("## graphx") == 1
...
def test_gemini_uninstall_removes_section(tmp_path):
    from graphx.__main__ import gemini_install, gemini_uninstall
    gemini_install(tmp_path)
...
def test_gemini_uninstall_removes_hook(tmp_path):
    ...
    if settings_path.exists():
        ...
        assert not any("graphx" in str(h) for h in hooks)

def test_gemini_uninstall_noop_if_not_installed(tmp_path):
    from graphx.__main__ import gemini_uninstall
