...
from graphx.watch import _notify_only, _WATCHED_EXTENSIONS
...
def test_notify_only_creates_flag(tmp_path):
    ...
    flag = tmp_path / "graphx-out" / "needs_update"
    assert flag.exists()
    assert flag.read_text() == "1"

def test_notify_only_creates_flag_dir(tmp_path):
    # graphx-out dir does not exist yet
    assert not (tmp_path / "graphx-out").exists()
    _notify_only(tmp_path)
    assert (tmp_path / "graphx-out").is_dir()
...
def test_watch_raises_without_watchdog(tmp_path, monkeypatch):
    ...
    from graphx.watch import watch
