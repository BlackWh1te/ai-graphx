...
graphx processes your files in three passes:
...
| graphx source + Transformer paper | 4 | **5.4x** |
...
Every extracted file is fingerprinted by content hash. Re-runs skip unchanged files entirely — only new or modified files go through extraction again. The cache lives in `graphx-out/cache/`.
