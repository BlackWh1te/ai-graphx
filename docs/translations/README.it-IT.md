...
**Una skill per assistenti di codice IA.** Scrivi `/graphx` in Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, VS Code Copilot Chat, Aider, OpenClaw, Factory Droid, Trae, Hermes, Kiro o Google Antigravity — legge i tuoi file, costruisce un grafo della conoscenza e ti restituisce struttura che non sapevi esistesse. Comprendi una codebase più velocemente. Trova il "perché" dietro le decisioni architetturali.

Completamente multimodale. Aggiungi codice, PDF, markdown, screenshot, diagrammi, foto di lavagne, immagini in altre lingue, o file video e audio — graphx estrae concetti e relazioni da tutto e li connette in un unico grafo. I video vengono trascritti localmente con Whisper. Supporta 25 linguaggi di programmazione via tree-sitter AST.

> Andrej Karpathy mantiene una cartella `/raw` dove deposita paper, tweet, screenshot e note. graphx è la risposta a quel problema — **71,5x** meno token per query rispetto alla lettura dei file grezzi, persistente tra le sessioni.

```
/graphx .                        # funziona con qualsiasi cartella
```
...
> **Pacchetto ufficiale:** Il pacchetto PyPI si chiama `graphx`. L'unico repository ufficiale è [safishamsi/graphx](https://github.com/safishamsi/graphx).

## Utilizzo

```
/graphx .
/graphx ./raw --update           # solo file modificati
/graphx ./raw --mode deep
/graphx query "cosa connette Attention all'ottimizzatore?"
...
## Costruito su graphx — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) è il livello enterprise su graphx. **Prova gratuita in arrivo.** [Unisciti alla lista d'attesa →](https://safishamsi.github.io/penpax.ai)
