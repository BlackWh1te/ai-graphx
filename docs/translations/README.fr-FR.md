...
**Une compétence pour assistant de code IA.** Tapez `/graphx` dans Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, VS Code Copilot Chat, Aider, OpenClaw, Factory Droid, Trae, Hermes, Kiro ou Google Antigravity — il lit vos fichiers, construit un graphe de connaissances et vous révèle une structure que vous ne voyiez pas auparavant. Comprenez une base de code plus rapidement. Trouvez le « pourquoi » derrière les décisions architecturales.

Entièrement multimodal. Déposez du code, des PDFs, du markdown, des captures d'écran, des diagrammes, des photos de tableau blanc, des images dans d'autres langues, ou des fichiers vidéo et audio — graphx extrait les concepts et les relations de tout cela et les connecte en un seul graphe. Les vidéos sont transcrites localement avec Whisper grâce à un prompt adapté au domaine. 25 langages de programmation supportés via tree-sitter AST (Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia, Verilog, SystemVerilog, Vue, Svelte, Dart).

> Andrej Karpathy maintient un dossier `/raw` où il dépose des articles, tweets, captures d'écran et notes. graphx est la réponse à ce problème — 71,5 fois moins de tokens par requête versus la lecture des fichiers bruts, persistant entre les sessions, honnête sur ce qui a été trouvé versus déduit.

```
/graphx .                        # fonctionne sur n'importe quel dossier — code, notes, articles, tout
```
...
Ajoutez un fichier `.graphxignore` pour exclure des dossiers :
...
> **Package officiel :** Le package PyPI s'appelle `graphx` (installer avec `pip install graphx`). Les autres packages nommés `graphx*` sur PyPI ne sont pas affiliés à ce projet. Le seul dépôt officiel est [safishamsi/graphx](https://github.com/safishamsi/graphx).
...
| Claude Code (Windows) | `graphx install` (détection automatique) ou `graphx install --platform windows` |
...
Note : Codex utilise `$` au lieu de `/` pour les compétences, tapez donc `$graphx .`.
...
```
/graphx                          # répertoire courant
/graphx ./raw                    # dossier spécifique
/graphx ./raw --mode deep        # extraction d'arêtes INFERRED plus agressive
/graphx ./raw --update           # ne réextraire que les fichiers modifiés
/graphx ./raw --directed         # graphe dirigé
/graphx ./raw --cluster-only     # relancer le clustering sur le graphe existant
/graphx ./raw --no-viz           # pas d'HTML, juste rapport + JSON
/graphx ./raw --obsidian         # générer un vault Obsidian (opt-in)

/graphx add https://arxiv.org/abs/1706.03762   # récupérer un article
/graphx add <video-url>                         # télécharger l'audio, transcrire, ajouter
/graphx query "qu'est-ce qui connecte Attention à l'optimiseur ?"
/graphx path "DigestAuth" "Response"
/graphx explain "SwinTransformer"

graphx hook install              # installer les hooks Git
graphx update ./src              # réextraire les fichiers de code, sans LLM
graphx watch ./src               # mise à jour automatique du graphe
```
...
**Hooks Git** (`graphx hook install`) — installe des hooks post-commit et post-checkout.

## Confidentialité

graphx envoie le contenu des fichiers à l'API du modèle de votre assistant IA pour l'extraction sémantique des docs, articles et images. Les fichiers de code sont traités localement via tree-sitter AST. Les fichiers vidéo et audio sont transcrits localement avec faster-whisper. Aucune télémétrie, aucun suivi d'utilisation.
...
## Construit sur graphx — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) est la couche enterprise au-dessus de graphx. Là où graphx transforme un dossier de fichiers en graphe de connaissances, Penpax applique le même graphe à toute votre vie professionnelle — en continu.
