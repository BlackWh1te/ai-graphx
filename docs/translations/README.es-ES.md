...
**Una habilidad para asistentes de código IA.** Escribe `/graphx` en Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, VS Code Copilot Chat, Aider, OpenClaw, Factory Droid, Trae, Hermes, Kiro o Google Antigravity — lee tus archivos, construye un grafo de conocimiento y te devuelve estructura que no sabías que existía. Entiende una base de código más rápido. Encuentra el «por qué» detrás de las decisiones arquitectónicas.

Totalmente multimodal. Deposita código, PDFs, markdown, capturas de pantalla, diagramas, fotos de pizarras, imágenes en otros idiomas, o archivos de video y audio — graphx extrae conceptos y relaciones de todo ello y los conecta en un solo grafo. Los videos se transcriben localmente con Whisper usando un prompt adaptado al dominio derivado de tu corpus. 25 lenguajes de programación soportados mediante tree-sitter AST (Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia, Verilog, SystemVerilog, Vue, Svelte, Dart).

> Andrej Karpathy mantiene una carpeta `/raw` donde deposita papers, tweets, capturas de pantalla y notas. graphx es la respuesta a ese problema — 71,5 veces menos tokens por consulta versus leer los archivos sin procesar, persistente entre sesiones, honesto sobre lo que encontró versus lo que infirió.

```
/graphx .                        # funciona con cualquier carpeta — tu código, notas, papers, todo
```
...
Añade un archivo `.graphxignore` para excluir carpetas:
...
> **Paquete oficial:** El paquete PyPI se llama `graphx` (instalar con `pip install graphx`). Otros paquetes llamados `graphx*` en PyPI no están afiliados con este proyecto. El único repositorio oficial es [safishamsi/graphx](https://github.com/safishamsi/graphx).
...
| Claude Code (Windows) | `graphx install` (detección automática) o `graphx install --platform windows` |
...
Nota: Codex usa `$` en lugar de `/` para habilidades, así que escribe `$graphx .`.
...
```
/graphx                          # directorio actual
/graphx ./raw                    # carpeta específica
/graphx ./raw --mode deep        # extracción de aristas INFERRED más agresiva
/graphx ./raw --update           # re-extraer solo archivos modificados
/graphx ./raw --directed         # grafo dirigido
/graphx ./raw --cluster-only     # re-ejecutar clustering en grafo existente
/graphx ./raw --no-viz           # sin HTML, solo informe + JSON
/graphx ./raw --obsidian         # generar vault de Obsidian (opt-in)

/graphx add https://arxiv.org/abs/1706.03762   # obtener un paper
/graphx add <video-url>                         # descargar audio, transcribir, añadir
/graphx query "¿qué conecta Attention con el optimizador?"
/graphx path "DigestAuth" "Response"
/graphx explain "SwinTransformer"

graphx hook install              # instalar hooks de Git
graphx update ./src              # re-extraer archivos de código, sin LLM
graphx watch ./src               # actualización automática del grafo
```
...
**Hooks de Git** (`graphx hook install`) — instala hooks post-commit y post-checkout.

## Privacidad

graphx envía contenido de archivos a la API del modelo de tu asistente IA para extracción semántica de documentos, papers e imágenes. Los archivos de código se procesan localmente mediante tree-sitter AST. Los archivos de video y audio se transcriben localmente con faster-whisper. Sin telemetría, sin seguimiento de uso.
...
## Construido sobre graphx — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) es la capa enterprise sobre graphx. Donde graphx convierte una carpeta de archivos en un grafo de conocimiento, Penpax aplica el mismo grafo a toda tu vida laboral — continuamente.
