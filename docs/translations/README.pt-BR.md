...
Adicione um arquivo `.graphxignore` para excluir pastas:
...
> **Pacote oficial:** O pacote PyPI chama-se `graphx` (instalar com `pip install graphx`). Outros pacotes chamados `graphx*` no PyPI não são afiliados a este projeto. O único repositório oficial é [safishamsi/graphx](https://github.com/safishamsi/graphx).
...
| Claude Code (Windows) | `graphx install` (detecção automática) ou `graphx install --platform windows` |
...
Nota: Codex usa `$` em vez de `/` para habilidades, então digite `$graphx .`.
...
```
/graphx                          # diretório atual
/graphx ./raw                    # pasta específica
/graphx ./raw --mode deep        # extração de arestas INFERRED mais agressiva
/graphx ./raw --update           # re-extrair apenas arquivos modificados
/graphx ./raw --directed         # grafo dirigido
/graphx ./raw --cluster-only     # re-executar clustering no grafo existente
/graphx ./raw --no-viz           # sem HTML, apenas relatório + JSON
/graphx ./raw --obsidian         # gerar vault do Obsidian (opt-in)

/graphx add https://arxiv.org/abs/1706.03762   # buscar um paper
/graphx add <video-url>                         # baixar áudio, transcrever, adicionar
/graphx query "o que conecta Attention ao otimizador?"
/graphx path "DigestAuth" "Response"
/graphx explain "SwinTransformer"

graphx hook install              # instalar hooks do Git
graphx update ./src              # re-extrair arquivos de código, sem LLM
graphx watch ./src               # atualização automática do grafo
```
...
**Hooks do Git** (`graphx hook install`) — instala hooks post-commit e post-checkout.

## Privacidade

graphx envia conteúdo de arquivos para a API do modelo do seu assistente IA para extração semântica de documentos, papers e imagens. Arquivos de código são processados localmente via tree-sitter AST. Arquivos de vídeo e áudio são transcritos localmente com faster-whisper. Sem telemetria, sem rastreamento de uso.
...
## Construído sobre graphx — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) é a camada enterprise sobre o graphx. Onde o graphx transforma uma pasta de arquivos em um grafo de conhecimento, o Penpax aplica o mesmo grafo a toda a sua vida profissional — continuamente.
