...
def _check_tree_sitter_version() -> None:
    ...
    if LANGUAGE_VERSION < 14:
        ...
        raise RuntimeError(
            ...
            f"graphx requires tree-sitter >= 0.23.0 (Language API v2). "
...
def extract(
    ...
    *,
    ...
    Args:
        ...
        cache_root: explicit root for graphx-out/cache/ (overrides the
            inferred common path prefix). Pass Path('.') when running on a
            subdirectory so the cache stays at ./graphx-out/cache/.
...
def collect_files(target: Path, *, follow_symlinks: bool = False, root: Path | None = None) -> list[Path]:
    ...
    from graphx.detect import _load_graphxignore, _is_ignored
    ignore_root = root if root is not None else target
    patterns = _load_graphxignore(ignore_root)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m graphx.extract <file_or_dir> ...", file=sys.stderr)
        sys.exit(1)
