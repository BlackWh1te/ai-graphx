...
Before running any subcommand below (`--update`, `--cluster-only`, `query`, `path`, `explain`, `add`), check that `.graphx_python` exists. If it's missing (e.g. user deleted `graphx-out/`), re-resolve the interpreter first:
