...
```powershell
# Detect Python and install graphx if needed
python -c "import graphx" 2>$null
if ($LASTEXITCODE -ne 0) { pip install graphx -q 2>&1 | Select-Object -Last 3 }
# Write interpreter path for all subsequent steps
python -c "import sys; open('.graphx_python', 'w').write(sys.executable)"
```
...
Set it as `$env:GRAPHX_WHISPER_PROMPT` before running the transcription command.

**Step 2 - Transcribe (PowerShell):**

```powershell
& (Get-Content graphx-out\.graphx_python) -c "
import json, os
from pathlib import Path
...
" | Out-File -FilePath graphx-out\.graphx_transcripts.json -Encoding utf8
```
...
**Whisper model:** Default is `base`. If the user passed `--whisper-model <name>`, set `$env:GRAPHX_WHISPER_MODEL = "<name>"` before running the command above.
...
- Load `total_words` and file counts from `.graphx_detect.json`
...
Only dispatch subagents for files listed in `.graphx_uncached.txt`. If all files are cached, skip to Part C directly.

**Step B1 - Split into chunks**

Load files from `.graphx_uncached.txt`. Split into chunks of 20-25 files each. Each image gets its own chunk (vision needs separate context).
...
"
```
Clean up temp files: `Remove-Item -ErrorAction SilentlyContinue .graphx_cached.json, .graphx_uncached.txt, .graphx_semantic_new.json`
...
```powershell
New-Item -ItemType Directory -Force -Path graphx-out | Out-Null
python -c "
import sys, json
...
Read `.graphx_analysis.json`. For each community key, look at its node labels and write a 2-5 word plain-language name (e.g. "Attention Mechanism", "Training Pipeline", "Data Loading").
...
"
Remove-Item -ErrorAction SilentlyContinue .graphx_detect.json, .graphx_extract.json, .graphx_ast.json, .graphx_semantic.json, .graphx_analysis.json, .graphx_labels.json
Remove-Item -ErrorAction SilentlyContinue graphx-out/.needs_update
```
...
result = json.loads(open('.graphx_incremental.json').read()) if Path('.graphx_incremental.json').exists() else {}
...
incremental = json.loads(Path('.graphx_incremental.json').read_text(encoding='utf-8'))
...
Before the merge step, save the old graph: `Copy-Item graphx-out/graph.json .graphx_old.json`
Clean up after: `Remove-Item -ErrorAction SilentlyContinue .graphx_old.json`
...
```powershell
python -m graphx save-result --question "QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
...
```powershell
python -m graphx save-result --question "Path from NODE_A to NODE_B" --answer "ANSWER" --type path_query --nodes NODE_A NODE_B
```
...
```powershell
python -m graphx save-result --question "Explain NODE_NAME" --answer "ANSWER" --type explain --nodes NODE_NAME
```
...
```powershell
python -m graphx.watch INPUT_PATH --debounce 3
```
...
If vertical scrolling breaks in PowerShell after running graphx, this is caused by ANSI escape sequences from the `graspologic` library. GraphX v0.3.10+ suppresses this output, but if you still see the issue:

1. **Upgrade graphx**: `pip install --upgrade graphx`
...
4. **Skip graspologic**: uninstall it (`pip uninstall graspologic`) and graphx will fall back to NetworkX's built-in Louvain algorithm, which produces no ANSI output
