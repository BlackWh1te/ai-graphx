...
def _load_graph(graph_path: str) -> nx.Graph:
    try:
        ...
        print(f"error: graph.json is corrupted ({exc}). Re-run /graphx to rebuild.", file=sys.stderr)
        sys.exit(1)
...
def serve(graph_path: str = "graphx-out/graph.json") -> None:
...
if __name__ == "__main__":
    graph_path = sys.argv[1] if len(sys.argv) > 1 else "graphx-out/graph.json"
    serve(graph_path)
