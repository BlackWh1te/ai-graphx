...
def build_from_json(extraction: dict, *, directed: bool = False) -> nx.Graph:
    ...
    for node in extraction.get("nodes", []):
        if isinstance(node, dict) and "source" in node and "source_file" not in node:
            ...
            print(
                f"[graphx] WARNING: node '{node_id}' uses field 'source' instead of "
    ...
    if real_errors:
        print(f"[graphx] Extraction warning ({len(real_errors)} issues): {real_errors[0]}", file=sys.stderr)
...
def deduplicate_by_label(nodes: list[dict], edges: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    print(f"[graphx] Deduplicated {len(remap)} duplicate node(s) by label.", file=sys.stderr)
...
def build_merge(
    ...
    graph_path: str | Path = "graphx-out/graph.json",
    ...
    *,
    ...
    if prune_sources:
        ...
        if to_remove:
            print(f"[graphx] Pruned {len(to_remove)} node(s) from deleted sources.", file=sys.stderr)
    ...
    if graph_path.exists():
        ...
        if new_n < existing_n:
            raise ValueError(
                f"graphx: build_merge would shrink graph from {existing_n} → {new_n} nodes. "
