...
```
/graphx                     # full pipeline on current directory
/graphx <path>              # full pipeline on specific path
/graphx <path> --update     # incremental - re-extract only new/changed files
/graphx <path> --no-viz     # skip visualization, just report + JSON
/graphx <path> --wiki       # build agent-crawlable wiki
/graphx query "<question>"  # BFS traversal - broad context
```
...
```
You are a graphx extraction subagent. Read the files listed and extract a knowledge graph fragment.
...
try:
    to_html(G, communities, 'graphx-out/graph.html')
...
Read `graphx-out/GRAPH_REPORT.md` and share the **God Nodes** and **Surprising Connections** sections directly in the chat — do not ask the user to open the file themselves.
