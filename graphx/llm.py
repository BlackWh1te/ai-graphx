...
_EXTRACTION_SYSTEM = """\
You are a graphx semantic extraction agent. Extract a knowledge graph fragment from the files provided.
...
def _parse_llm_json(raw: str) -> dict:
    ...
    try:
        ...
        print(f"[graphx] LLM returned invalid JSON, skipping chunk: {exc}", file=sys.stderr)
...
def _extract_with_adaptive_retry(
    ...
    if len(chunk) <= 1:
        print(
            f"[graphx] single-file chunk {chunk[0]} truncated at "
    ...
    if _depth >= max_depth:
        print(
            f"[graphx] chunk of {len(chunk)} still truncated at recursion "
    ...
    print(
        f"[graphx] chunk of {len(chunk)} truncated at depth {_depth}, "
...
def extract_corpus_parallel(
    ...
    if workers == 1:
        ...
        for idx, chunk in enumerate(chunks):
            ...
            if exc is not None:
                print(f"[graphx] chunk {idx + 1}/{total} failed: {exc}", file=sys.stderr)
                continue
