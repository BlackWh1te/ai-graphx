...
Set it as `GRAPHX_WHISPER_PROMPT` in the environment before running the transcription command.
...
Wait for all agents to return. Parse each response as JSON. Accumulate nodes/edges/hyperedges across all results and write to `graphx-out/.graphx_semantic_new.json`.
...
n = to_obsidian(G, communities, 'graphx-out/obsidian', community_labels=labels or None, cohesion=cohesion)
print(f'Obsidian vault: {n} notes in graphx-out/obsidian/')

to_canvas(G, communities, 'graphx-out/obsidian/graph.canvas', community_labels=labels or None)
print('Canvas: graphx-out/obsidian/graph.canvas - open in Obsidian for structured community layout')
print()
print('Open graphx-out/obsidian/ as a vault in Obsidian.')
...
Run this before Step 9 (cleanup) so `graphx-out/.graphx_labels.json` is still available.
...
"
rm -f graphx-out/.graphx_detect.json graphx-out/.graphx_extract.json graphx-out/.graphx_ast.json graphx-out/.graphx_semantic.json graphx-out/.graphx_analysis.json graphx-out/.graphx_labels.json graphx-out/.graphx_chunk_*.json
