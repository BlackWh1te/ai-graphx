...
Load files from `.graphx_uncached.txt`. Split into chunks of 20-25 files each. Each image gets its own chunk (vision needs separate context). When splitting, group files from the same directory together so related artifacts land in the same chunk and cross-file relationships are more likely to be extracted.
...
Accumulate nodes/edges/hyperedges across all results and write to `.graphx_semantic_new.json`.
...
If `code_only` is True: print `[graphx update] Code-only changes detected - skipping semantic extraction (no LLM needed)`, run only Step 3A (AST) on the changed files, skip Step 3B entirely, then go straight to merge and Steps 4–8.
...
Before the merge step, save the old graph: `cp graphx-out/graph.json .graphx_old.json`
Clean up after: `rm -f .graphx_old.json`
...
Run once per project to make graphx always-on in Trae sessions:

```bash
graphx trae install       # or: graphx trae-cn install
```

This writes a `## graphx` section to the local `AGENTS.md` that instructs Trae to check the graph before answering codebase questions and rebuild it after code changes. No manual `/graphx` needed in future sessions.

> **Note:** Unlike Claude Code, Trae does NOT support PreToolUse hooks. The AGENTS.md rules are the always-on mechanism — there is no automatic graph rebuild on tool use. Run `/graphx --update` manually after code changes if the graph needs refreshing.

```bash
graphx trae uninstall     # or: graphx trae-cn uninstall   # remove the section
```
