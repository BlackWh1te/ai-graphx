...
from graphx.detect import CODE_EXTENSIONS, DOC_EXTENSIONS, PAPER_EXTENSIONS, IMAGE_EXTENSIONS
...
def _rebuild_code(watch_path: Path, *, follow_symlinks: bool = False, force: bool = False) -> bool:
    ...
    try:
        ...
        if not code_files:
            print("[graphx watch] No code files found - nothing to rebuild.")
            return False
        ...
        (out / ".graphx_root").write_text(str(watch_root), encoding="utf-8")
        ...
        try:
            ...
            print(f"[graphx watch] Skipped graph.html: {viz_err}")
        ...
        print(f"[graphx watch] Rebuilt: {G.number_of_nodes()} nodes, "
              f"{G.number_of_edges()} edges, {len(communities)} communities")
        products = "graph.json" + (", graph.html" if html_written else "") + ", index.html and GRAPH_REPORT.md"
        print(f"[graphx watch] {products} updated in {out}")
        return True

    except Exception as exc:
        print(f"[graphx watch] Rebuild failed: {exc}")
        return False
...
def check_update(watch_path: Path) -> bool:
    ...
    re-extraction via `/graphx --update` — this function only signals
    that the update is needed.
    """
    flag = Path(watch_path) / _GRAPHX_OUT / "needs_update"
    if flag.exists():
        print(f"[graphx check-update] Pending non-code changes in {watch_path}.")
        print("[graphx check-update] Run `/graphx --update` to apply semantic re-extraction.")
    return True


def _notify_only(watch_path: Path) -> None:
    """Write a flag file and print a notification (fallback for non-code-only corpora)."""
    flag = watch_path / _GRAPHX_OUT / "needs_update"
    flag.parent.mkdir(parents=True, exist_ok=True)
    flag.write_text("1", encoding="utf-8")
    print(f"\n[graphx watch] New or changed files detected in {watch_path}")
    print("[graphx watch] Non-code files changed - semantic re-extraction requires LLM.")
    print("[graphx watch] Run `/graphx --update` in Claude Code to update the graph.")
    print(f"[graphx watch] Flag written to {flag}")
...
def watch(watch_path: Path, debounce: float = 3.0) -> None:
    ...
    to run /graphx --update (LLM extraction required).
    ...
    print(f"[graphx watch] Watching {watch_path.resolve()} - press Ctrl+C to stop")
    print(f"[graphx watch] Code changes rebuild graph automatically. "
          f"Doc/image changes require /graphx --update.")
    print(f"[graphx watch] Debounce: {debounce}s")
    ...
    try:
        while True:
            ...
            if pending and (time.monotonic() - last_trigger) >= debounce:
                ...
                batch = list(changed)
                changed.clear()
                print(f"\n[graphx watch] {len(batch)} file(s) changed")
                if _has_non_code(batch):
                    _notify_only(watch_path)
                else:
                    _rebuild_code(watch_path)
    except KeyboardInterrupt:
        print("\n[graphx watch] Stopped.")
    finally:
        observer.stop()
        observer.join()
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Watch a folder and auto-update the graphx graph")
