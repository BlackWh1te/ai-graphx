...
_MANIFEST_PATH = "graphx-out/manifest.json"
...
def xlsx_extract_structure(path: Path) -> dict:
    ...
    Returns a nodes/edges dict compatible with the graphx extract pipeline.
...
_SKIP_DIRS = {
    ...
    ".tox", ".eggs", "*.egg-info",
    "graphx-out",  # never treat own output as source input (#524)
}
...
def _parse_gitignore_line(raw: str) -> str:
    """Parse one raw line from a .graphxignore file per gitignore spec.
...
def _load_graphxignore(root: Path) -> list[tuple[Path, str]]:
    """Read .graphxignore files and return (anchor_dir, pattern) pairs.
...
def _is_ignored(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if the path should be ignored per .graphxignore patterns.
...
def _load_graphxinclude(root: Path) -> list[tuple[Path, str]]:
    """Read .graphxinclude allowlist patterns from root and ancestors.
    ...
    Uses the same VCS-root ceiling logic as _load_graphxignore.
    """
    root = root.resolve()
    ceiling = _find_vcs_root(root) or root
...
def _is_included(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if path matches any .graphxinclude allowlist pattern."""
    if not patterns:
        return False
...
def _could_contain_included_path(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if a directory may contain files matched by .graphxinclude."""
    if not patterns:
        return False
...
def detect(root: Path, *, follow_symlinks: bool = False) -> dict:
    ...
    ignore_patterns = _load_graphxignore(root)
    include_patterns = _load_graphxinclude(root)

    # Always include graphx-out/memory/ - query results filed back into the graph
    memory_dir = root / "graphx-out" / "memory"
    scan_paths = [root]
    ...
    for scan_root in scan_paths:
        ...
        for dirpath, dirnames, filenames in os.walk(scan_root, followlinks=follow_symlinks):
            ...
            if not in_memory_tree:
                ...
                # explicitly included path (.graphxinclude allowlist).
    ...
    converted_dir = root / "graphx-out" / "converted"
    ...
    for p in all_files:
        ...
        if not in_memory:
            ...
            # but catch hidden files at the root level. A .graphxinclude
        ...
        if ftype:
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(str(p) + " [office conversion failed - pip install graphx[office]]")
                continue
    ...
    return {
        ...
        "graphxignore_patterns": len(ignore_patterns),
    }
