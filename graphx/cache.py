...
# Output directory name — override with GRAPHX_OUT env var for worktrees or
# shared-output setups. Accepts a relative name ("graphx-out-feature") or an
# absolute path ("/shared/graphx-out").
...
def cache_dir(root: Path = Path("."), kind: str = "ast") -> Path:
    """Returns graphx-out/cache/{kind}/ - creates it if needed.
...
def load_cached(path: Path, root: Path = Path("."), kind: str = "ast") -> dict | None:
    ...
    Cache value: stored as graphx-out/cache/{kind}/{hash}.json
    ...
    if kind == "ast":
        legacy = Path(root).resolve() / _GRAPHX_OUT / "cache" / f"{h}.json"
        if legacy.exists():
            try:
...
def save_cached(path: Path, result: dict, root: Path = Path("."), kind: str = "ast") -> None:
    ...
    Stores as graphx-out/cache/{kind}/{hash}.json where hash = SHA256 of current file contents.
...
def cached_files(root: Path = Path(".")) -> set[str]:
    ...
    base = Path(root).resolve() / _GRAPHX_OUT / "cache"
    hashes: set[str] = set()
    # Legacy flat entries
    if base.is_dir():
