...
- Feat: `GRAPHX_OUT` env var overrides the output directory — accepts a relative name or absolute path, wires through `cache.py`, `watch.py`, and the CLI; useful for sharing one graph across multiple git worktrees (#686)
- Fix: `graphx antigravity install` now auto-updates stale rules and workflow files on re-run instead of silently skipping them (#652)
- Docs: README simplified — less dense, plain language; technical pipeline details moved to `docs/how-it-works.md`

## 0.6.8 (2026-05-03)

- Fix: `.graphxignore` negation patterns (`!src/**`) now work correctly — when any `!` pattern is present, directory pruning is deferred to per-file checks so negated files inside ignored directories are reached (#676)
- Fix: Antigravity slash command `/graphx` now appears in the command dropdown — workflow file now includes YAML frontmatter with `name: graphx` required for Antigravity discovery (#678)
...
- Fix: `graphx install --platform codex` now writes absolute path to `graphx` executable — fixes PATH resolution in VS Code extension on Windows (#651)
...
- Feat: `graphx tree` — self-contained D3 v7 collapsible-tree HTML view of `graph.json`; expand/collapse controls, depth-based colours, hover inspector; XSS-safe (#557)
...
- Feat: `.graphxinclude` hidden path allowlist — opt specific hidden dirs into traversal (e.g. `.hermes/plans/**/*.md`) (#583)
- Feat: `--no-viz` flag wired in `cluster-only`; `GRAPHX_VIZ_NODE_LIMIT` env var overrides 5000-node HTML threshold (#565)
...
- Feat: `graphx tree` — self-contained D3 v7 collapsible-tree HTML view of `graph.json`; expand/collapse controls, depth-based colours, hover inspector; XSS-safe via `html.escape()` and `_js_safe()` (#557)
...
- Feat: `graphx update --force` and `GRAPHX_FORCE=1` env var — bypass the node-count safety check after refactors that legitimately shrink the graph (#639)
- Fix: Codex PreToolUse hook on Windows — replaced `python3 -c "..."` inline command (fails on Conda where only `python` exists, and breaks PowerShell JSON parsing) with `graphx hook-check`, a new shell-agnostic subcommand. Re-run `graphx codex install` to regenerate the hook (#651, #522)
...
- Fix: incremental rebuild (`graphx update`, post-commit hook) dropped INFERRED/AMBIGUOUS semantic nodes extracted from code files — node preservation now filters by ID membership in the new AST output instead of `file_type`, so LLM-extracted call/data-flow edges survive code-only rebuilds (#653)
- Fix: post-commit and post-checkout hooks blocked `git commit` for the full rebuild duration (hours on large repos) — rebuilds now detach via `nohup & disown`, git returns in ~100ms, log written to `~/.cache/graphx-rebuild.log` (#650)
...
- Fix: `graphx update` / `graphx watch` never persisted the manifest, so every subsequent `--update` re-extracted all files — manifest now saved after each rebuild (#621)
- Fix: inline comments in `.graphxignore` (e.g. `vendor/ # legacy`) now stripped correctly — whitespace + `#` suffix is treated as a comment, `path#hash.py` preserved (#605)
- Fix: `graphx query "FunctionName"` now returns the exact matching node first instead of high-degree hub modules hijacking the output — 100-point exact-match bonus + seeds render before BFS expansion (#638)
...
- Fix: `.graphxignore` negation patterns (`!src/lib/secrets.ts`) now work — full last-match-wins evaluation with `!` un-ignore support (#628)

## 0.6.1 (2026-05-01)

- Fix: `.graphxignore` discovery now uses correct gitignore semantics — outer rules are loaded first so inner (closer) rules always win via last-match-wins, matching standard gitignore behavior (#643)
- Fix: without a VCS root, `.graphxignore` discovery is now hermetic to the scan folder — no leakage across sibling projects in a shared workspace (#643)
- Fix: anchored patterns (leading `/`) in a parent `.graphxignore` now correctly apply only relative to their own directory, not the scan root (#643)
...
- Feat: SQL AST extractor — `.sql` files now processed deterministically via tree-sitter. Extracts tables, views, functions/procedures, foreign key references, and FROM/JOIN reads_from edges. No LLM needed. Requires `pip install 'graphx[sql]'` (#349)
...
- Fix: `NameError: name '_os' is not defined` crash after `graphx update` — this was fixed in v5 branch but not released to PyPI (#618, #612)

## 0.5.5 (2026-04-29)

- Feat: Kimi K2.6 backend — `pip install 'graphx[kimi]'` + `MOONSHOT_API_KEY` routes semantic extraction through Kimi K2.6. 3-6x richer relation extraction at ~3x lower cost. Claude remains default; Kimi is opt-in.
...
- Fix: `graphx update` remembers scan root via `graphx-out/.graphx_root` — no path argument needed on subsequent runs
...
- Fix: community labels deleted in Step 9 cleanup — `.graphx_labels.json` is now preserved so wiki/obsidian/HTML retain human-readable names after re-cluster (#608)
- Fix: `NameError: name '_os' is not defined` in `graphx update` Kimi tip (#612)
...
- Feat: `graphx clone <github-url>` — clone and graph any public repo
- Feat: `graphx merge-graphs` — combine multiple `graph.json` outputs into one cross-repo graph
- Feat: `CLAUDE_CONFIG_DIR` support in `graphx install`
...
- Fix: `graphx-out/` excluded from source scanning

## 0.4.23 (2026-04-18)

- Fix: stale skill version warning persists after running `graphx install` when multiple platforms were previously installed — `graphx install` now refreshes `.graphx_version` in all other known skill directories so the warning clears across the board (#178)
...
- Fix: AST cache written to `src/graphx-out/cache/` instead of project root when all code files share a common prefix like `src/` — `extract()` now called with explicit `cache_root=watch_path` in `_rebuild_code` and `cache_root=Path('.')` in the Codex skill AST step (#429)
...
- Fix: `graphx cluster-only` crashed with `KeyError: 'total_files'` in `report.py` — cluster-only skips detection so the stats dict was empty; now passes a `warning` key so the report skips the file-stats section (#422)
- Fix: `/graphx --update` dropped all existing graph nodes — the merge block built a correct in-memory `G_existing` but never wrote it back to `.graphx_extract.json`, so Step 4 rebuilt from the new-extraction-only file; merged result is now serialized back before Step 4 runs (#423)
...
- Fix: `graphx update .` and `graphx cluster-only` now generate `graph.html` alongside `graph.json` and `GRAPH_REPORT.md` — previously only the skill generated the interactive HTML (#418)
...
- Fix: Windows `graphx-out/graphx-out` nesting bug — `cache_dir` and `_rebuild_code` in watch.py now call `.resolve()` on the root path, preventing a nested output directory when graphx is run from a subdirectory (#410)
- Fix: `graphx hook install` now respects `core.hooksPath` git config (used by Husky and similar tools) — hooks are written to the configured path instead of always `.git/hooks` (#401)
...
- Docs: added team workflow section — committing `graphx-out/`, `.graphxignore` usage, and recommended `.gitignore` additions (#369)

## 0.4.16 (2026-04-16)

- Fix: graphx watch crashed on all platforms with NameError because import sys was missing from watch.py (#386, #394)
...
- Feat: VS Code Copilot Chat support — `graphx vscode install` installs a Python-only skill (works on Windows PowerShell) and writes `.github/copilot-instructions.md` for always-on graph context (#206)
...
- Fix: macOS `graphx watch` now uses `PollingObserver` by default to avoid missed events with FSEvents (#373)
...
- Fix: `--wiki` flag now runs `to_wiki()` as Step 6b in the skill pipeline before the cleanup step — community labels are available and the wiki is written to `graphx-out/wiki/` (#229, #354)
- Fix: `graphx install --platform opencode` now also installs the `.opencode/plugins/graphx.js` plugin, matching what `graphx opencode install` does (#356)
- Fix: `extract()` accepts explicit `cache_root` parameter so subdirectory runs no longer write cache to `<subdir>/graphx-out/cache/` (#350)
- Fix: `os.replace` in cache writer falls back to `shutil.copy2` on `PermissionError` (Windows WinError 5) (#287)
- Fix: `graphx update` exits with code 1 on rebuild failure instead of silently returning (#287)
- Fix: `CLAUDE.md`, Cursor, and Antigravity templates now use `graphx update .` instead of hardcoded `python3 -c` invocation (#287)
- Fix: `skill-kiro.md` added to `pyproject.toml` package-data — `graphx kiro install` was failing on fresh pip installs (#352)
...
- Fix: AGENTS.md and GEMINI.md rebuild rule now uses `graphx update .` instead of hardcoded `python3 -c "..."` — correct Python is resolved through the graphx binary, no more interpreter mismatches in Nix/pipx/uv environments (#324)
- Fix: `graphx query` and `graphx explain` no longer crash with `AttributeError` when a node has `label: null` — all `.get("label", "")` calls guarded with `or ""` to handle explicit null values (#323)

## 0.4.12 (2026-04-13)

- Add: Kiro IDE/CLI support — `graphx kiro install` writes `.kiro/skills/graphx/SKILL.md` (invoked via `/graphx`) and `.kiro/steering/graphx.md` (`inclusion: always` — always-on context before every conversation) (#319, #321)
...
- Fix: `graphx query` no longer crashes with `ValueError` on MultiGraph graphs — `G.edges[u, v]` replaced with `G[u][v]` + MultiGraph guard (#305)
- Fix: `graphx query` no longer crashes with `AttributeError: 'NoneType' has no attribute 'lower'` when a node has a null `source_file` (#307)
- Fix: MCP server launched from a different directory now correctly derives the `graphx-out` base from the absolute path provided, instead of CWD (#309)
- Fix: `.graphxignore` patterns from a parent directory now fire correctly when graphx is run on a subfolder — patterns are matched against paths relative to both the scan root and the `.graphxignore`'s anchor directory (#303)

## 0.4.10 (2026-04-13)

- Fix: `graphx install --platform cursor` no longer crashes — passes `Path(".")` to `_cursor_install` (#281)
...
- Add: Hermes Agent platform support — `graphx hermes install` writes skill to `~/.hermes/skills/graphx/SKILL.md` and AGENTS.md (#251)
...
- Fix: `graphx claw install` now writes to `.openclaw/` (correct OpenClaw directory) instead of `.claw/` (#208)
- Add: Blade template support — `@include`, `<livewire:>` components, and `wire:click` bindings extracted from `.blade.php` files (#242)
- Docs: WSL/Linux MCP setup note — package name is `graphx`, use `.venv/bin/python3` in `.mcp.json` (#250)

## 0.4.6 (2026-04-12)

- Add: Google Antigravity support — `graphx antigravity install` writes `.agent/rules/graphx.md` (always-on rules) and `.agent/workflows/graphx.md` (`/graphx` slash command) (#203, #199, #53)
...
- Fix: Windows hooks now fall back to `python` if `python3` is not found; exits cleanly if neither has graphx installed (#244)
...
- Fix: `collect_files()` in `extract.py` now respects `.graphxignore` — previously ignored patterns, causing thousands of unwanted files (e.g. `node_modules/`) to be scanned (#188)
...
- Add: drop `.mp4`, `.mp3`, `.wav`, `.mov`, `.webm`, `.m4a`, `.ogg`, `.mkv`, `.avi`, `.m4v` files into any corpus and graphx transcribes them locally with faster-whisper before extraction
- Add: YouTube and URL download via yt-dlp — `/graphx add https://youtube.com/...` downloads audio-only and feeds it through the same Whisper pipeline
...
- Add: `graphx-out/transcripts/` cache — transcripts cached by filename; YouTube URLs cached by hash so re-runs skip already-transcribed files
- Requires: `pip install 'graphx[video]'` for faster-whisper and yt-dlp

## 0.3.29 (2026-04-10)

- Add: video and audio corpus support — drop `.mp4`, `.mp3`, `.wav`, `.mov`, `.webm`, `.m4a`, `.ogg`, `.mkv`, `.avi`, `.m4v` files into any corpus and graphx transcribes them with faster-whisper before extraction
- Add: YouTube and URL video download — pass a YouTube link (or any video URL) to `/graphx add <url>` and yt-dlp downloads audio-only, which is then transcribed and added to the corpus automatically
...
- Add: `graphx-out/transcripts/` cache — transcripts are cached by filename so re-runs skip already-transcribed files; URLs cached by hash
- Requires: `pip install 'graphx[video]'` for faster-whisper + yt-dlp
...
- Fix: graphx install --platform gemini now also copies the skill file to ~/.gemini/skills/graphx/SKILL.md so the /graphx trigger works in Gemini CLI (#174)
...
- Fix: `graphx install --platform gemini` now routes to `gemini_install()` instead of erroring — `gemini` was missing from `_PLATFORM_CONFIG` (#171)
- Fix: `graphx install --platform cursor` now routes to `_cursor_install()` the same way (#171)
...
- Fix: `_load_graphxignore` now walks parent directories up to the `.git` boundary, matching `.gitignore` discovery behavior — subdirectory scans now inherit root ignore patterns (#168)
- Add: Aider platform support — `graphx install --platform aider` copies skill to `~/.aider/graphx/SKILL.md`; `graphx aider install/uninstall` writes AGENTS.md rules (#74)
- Add: GitHub Copilot CLI platform support — `graphx install --platform copilot` copies skill to `~/.copilot/skills/graphx/SKILL.md`; `graphx copilot install/uninstall` for skill management (#134)
...
- Fix: `graphx codex install` (and opencode) no longer exits early when `AGENTS.md` already has the graphx section — partial installs with a missing `.codex/hooks.json` can now recover on re-run (#153)

## 0.3.23 (2026-04-09)

- Add: Gemini CLI support — `graphx gemini install` writes a `GEMINI.md` section and a `BeforeTool` hook in `.gemini/settings.json` that fires before file-read tool calls (#105)
...
- Add: Cursor support — `graphx cursor install` writes `.cursor/rules/graphx.mdc` with `alwaysApply: true` so the graph context is always included; `graphx cursor uninstall` removes it (#137)
...
- Fix: README clarifies `graphx` is the only official PyPI package — other `graphx*` packages are not affiliated (#129)
...
- Add: OpenCode `tool.execute.before` plugin — `graphx opencode install` now writes `.opencode/plugins/graphx.js` and registers it in `opencode.json`, firing the graph reminder before bash calls (equivalent to Claude Code's PreToolUse hook) (#71)
...
- Fix: `--watch` mode now respects `.graphxignore` — `_rebuild_code` was calling `collect_files()` directly instead of `detect()`, bypassing ignore patterns (#120)
...
- Refactor: `save_query_result` inline Python blocks in all 6 skill files replaced with `graphx save-result` CLI command — shorter, maintainable, less tokens for LLM (#114)
- Add: `graphx save-result` CLI subcommand — saves Q&A results to memory dir without inline Python
...
- Fix: `mkdir -p graphx-out` now runs before writing `.graphx_python` in `skill.md` — prevents write failure on first run; `.graphx_python` no longer deleted in Step 9 cleanup across all skill files so follow-up commands keep their interpreter (#93)
...
- Fix: `graphx query`, `serve`, and `benchmark` now work on NetworkX < 3.4 — version-safe shim for `node_link_graph()` at all call sites (#95)
- Fix: `.jsx` files now detected and extracted via the JS extractor — added to `CODE_EXTENSIONS` and `_DISPATCH` (#94)
- Fix: `.graphx_python` no longer deleted in Step 9 cleanup across all 6 skill files — pipx users no longer hit `ModuleNotFoundError` on follow-up commands (#92)

## 0.3.15 (2026-04-08)

- Feat: Trae and Trae CN platform support (`graphx install --platform trae` / `trae-cn`)
...
- Fix: pipx installs now detected correctly in git hooks (reads shebang from graphx binary)
...
- Fix: `graphx codex install` now also writes a PreToolUse hook to `.codex/hooks.json` so the graph reminder fires before every Bash tool call (#86)
...
- Fix: Skill intermediate files (`.graphx_*.json`) now live in `graphx-out/` instead of project root, preventing git pollution (#81)
...
- Fix: Windows UnicodeEncodeError during `graphx install` — replaced arrow character with `->` in all print statements (#47)
...
- Docs: Codex uses `$graphx .` not `/graphx .` (#36)
...
- Add: `graphx query "<question>"` CLI command — BFS/DFS traversal of `graph.json` without needing Claude Code skill (`--dfs`, `--budget N`, `--graph <path>` flags)
...
- Add: `--obsidian-dir <path>` flag — write Obsidian vault to a custom directory instead of `graphx-out/obsidian`
...
- Add: `graphx install --platform <codex|opencode|claw>` routes skill to correct config directory
- Add: `graphx codex install` / `opencode install` / `claw install` — writes AGENTS.md for always-on graph-first behaviour
- Add: `graphx claude uninstall` / `codex uninstall` / `opencode uninstall` / `claw uninstall`
- Add: MIT license
...
- Add: `graphx claude install` — writes graphx section to local CLAUDE.md + PreToolUse hook in `.claude/settings.json`
- Add: `graphx claude uninstall` — removes section and hook
- Add: `graphx hook install` — installs post-commit and post-checkout git hooks (platform-agnostic)
- Add: `graphx hook uninstall` / `hook status`
- Add: `graphx benchmark` CLI command
- Fix: node deduplication documented at all three layers

## 0.1.8 (2026-04-05)

- Fix: follow-up questions now check for wiki first (graphx-out/wiki/index.md) before falling back to graph.json
- Fix: --update now auto-regenerates wiki if graphx-out/wiki/ exists
- Fix: community articles show truncation notice ("... and N more nodes") when > 25 nodes
- UX: pipeline completion message now lists all available flags and commands so users know what graphx can do
...
- Add: `graphx/wiki.py` module with `to_wiki()` — cross-community wikilinks, cohesion scores, audit trail, navigation footer
...
- Fix: `pip install graphx` → `pip install graphx` in skill Step 1 (critical install bug)
...
- Docs: README reframed as Claude Code skill; Karpathy problem → graphx answer framing
